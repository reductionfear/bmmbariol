...
let currentAck = 0;
// --- WebSocket Capture via prototype intercept ---
// This captures Lichess's WebSocket even if it was created before our script loaded
const _originalWsSend = WebSocket.prototype.send;
WebSocket.prototype.send = function(data) {
  if (this.url && this.url.includes('socket') && !webSocketWrapper) {
    console.log('[WS] ✅ Captured WebSocket via send()');
    webSocketWrapper = this;
  ...
  return _originalWsSend. call(this, data);
};
...
window.addEventListener('message', (e) => {
  ...
  if (typeof updateExtEngineBtn === 'function') updateExtEngineBtn(); // <-- ADD THIS LINE
});
...
function executeMove(uci) {
  ...
  if (clockSecs > 2.0 && uci === lastMoveSent && (now - lastMoveSentTime) < 100) {
...
async function processTurn() {
  ...
  try {
	...
	// Ensure isProcessing is always reset after a timeout
	setTimeout(() => {
		if (isProcessing) {
		  console.log('[ProcessTurn] Lock timeout - force resetting');
		  isProcessing = false;
		  pendingMove = false;
		}
...
async function run() {
  ...
  const moveObs = new MutationObserver((muts) => {
	  for (const mut of muts) {
		...
		// FULL SYNC instead of just last move
		syncGameState();
  ...
  waitForElement('rm6').then((el) => {
	  ...
	  syncGameState();
	  setTimeout(processTurn, 500);
	  // ADD THIS - Auto-connect external engine if enabled
	  if (useExternalEngine) {
		console.log('[ExtEngine] Auto-connecting on page load...');
		setTimeout(() => {
		  connectExternalEngine();
		}, 500);
	  }
